...
import { sendEmailForOTP } from "@root/actions/mail/auth";

const SignupForm = () => {
    const [state, submitAction] = useActionState(userSignup, null);
    const [otpFormState, binder] = useActionState(sendEmailForOTP, null);
    ...
    }, [router, state, otpValidated]);

    useEffect(() => {
        if (otpFormState?.status !== 200) {
            toast.error(otpFormState?.message);
        }
        else if (otpFormState?.status === 200) {
            document.getElementById("signUpForm").classList.toggle("hidden");
            document.getElementById("otpForm").classList.toggle("hidden");
            toast.success(otpFormState?.message, {
                onClick: () => {
            ...
            <div className="w-full h-screen bg-gray-100 lg:w-1/2">
                <div className="flex min-h-full flex-col justify-center">
                    ...
                    <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                        <form action={binder} className="space-y-8" id="signUpForm" >
                            ...
                            <SubmitButton title="Send OTP" size="full" />
                        </form>
                        <form className="mt-3 py-2 border-t border-t-gray-600 hidden"
                            action={submitAction} id="otpForm"
                        >
                            ...
                            <OTPInput
                                ...
                                focusStyle={{ border: "1px solid #000" }}
                            />
                            <input type="hidden" name="otp" value={otp} />
                            <input type="hidden" name="name" value={otpFormState?.name} />
                            <input type="hidden" name="email" value={otpFormState?.email} />
